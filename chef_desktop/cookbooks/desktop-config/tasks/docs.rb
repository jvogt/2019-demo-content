namespace :docs_site do
  ...
  task :resources do
    ...
    def build_menu_item(name)
      {
        'desktop' => {
          'title' => name,
          'identifier' => "desktop/resources/#{name} #{name}",
    ...
    def special_properties(name, _data)
      properties = {}
    ...
    FileUtils.mkdir_p 'auto_generated_docs'
    ...
    resources.each do |resource, data|
      ...
      if ENV['DEBUG']
        ...
        FileUtils.mkdir_p "auto_generated_docs/#{resource}"
        ...
        File.open("auto_generated_docs/#{resource}/index.md", 'w') { |f| f.write(resource_data.to_yaml + '---') }
      end
    end
  end
end
